...
function dateOnlyAfterDuration(start: string, durationMs: number): string {
...
function buildEvent(props: ICalProp[], calendarName: string, currentUserEmail?: string): CalendarEvent | null {
  ...
  if (!dtend && duration) {
    const durationMs = parseICalDuration(duration.value);
    if (durationMs !== null) {
      end = isAllDay
        ? dateOnlyAfterDuration(start, durationMs)
        : formatUtcDateTime(new Date(start).getTime() + durationMs);
    }
  }
