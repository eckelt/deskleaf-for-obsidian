...
/** Any single status character between the brackets — Tasks-plugin statuses are one char. */
export const TODO_STATUS_PATTERN = /^(\s*[-*]\s+)\[(.)\]\s+(.*\S)\s*$/;
...
/** `x`/`X`/`-` are done/cancelled (closed), `!` is important, everything else is open. */
export function classifyTodoStatus(char: string): TodoStatus {
...
export function parseTodoLines(content: string): ParsedTodo[] {
  ...
  for (let index = 0; index < lines.length; index++) {
    const match = TODO_STATUS_PATTERN.exec(lines[index]);
    if (!match) continue;
    todos.push({
      text: cleanTodoText(match[3]),
      raw: match[3],
      status: classifyTodoStatus(match[2]),
...
export function completeTodoLine(line: string, today: string): string {
  const match = TODO_STATUS_PATTERN.exec(line);
  if (!match || classifyTodoStatus(match[2]) === "closed") return line;
...
/** An important todo always lands in its own group, regardless of due date. */
export function groupForDate(status: TodoStatus, date: string | null, today: string, weekEnd: string): TodoGroup {
