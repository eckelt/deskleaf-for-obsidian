...
import {
  TODO_LINE_REGEX, classifyTodoStatus, resolveTodoDate, resolveTodoGroup, setTodoLineChecked,
  type TodoStatus, type TodoGroup,
} from "./todo-utils";
...
export class DeskleafSidebarView extends ItemView {
  ...
  private parseTodosFromFile(file: TFile, content: string): TodoItem[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      const match = TODO_LINE_REGEX.exec(lines[i]);
      if (!match) continue;
      const text = match[2];
      todos.push({
        text,
        status: classifyTodoStatus(match[1]),
        file,
        lineIndex: i,
  ...
  private groupTodos(todos: TodoItem[]): Record<TodoGroup, TodoItem[]> {
    ...
    for (const todo of todos) {
      const group = resolveTodoGroup(todo.status, todo.date, today, weekEnd);
      if (group) groups[group].push(todo);
    }
    return groups;
  }
  ...
  private async toggleTodo(todo: TodoItem, checked: boolean) {
    ...
    const lines = content.split("\n");
    lines[todo.lineIndex] = setTodoLineChecked(lines[todo.lineIndex], checked);
