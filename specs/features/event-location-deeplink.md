...
## User Story
Als Nutzer moechte ich, dass beim Oeffnen einer Brain-Notiz fuer einen Termin ohne bestehenden Location-Eintrag automatisch ein Obsidian-Deeplink zu dieser Notiz ins Location-Feld des Kalendertermins geschrieben wird, damit Menueleisten-Apps wie meetingbar direkt zur Notiz statt zu einem Online-Meeting springen koennen.

## Acceptance Criteria
- [ ] AC1: Wird ueber `openEvent`/`openOrCreate` eine Notiz fuer einen Termin geoeffnet und ist `event.location` zu diesem Zeitpunkt leer oder nur Whitespace, schreibt das Plugin danach automatisch einen Obsidian-Deeplink zu dieser Notiz ins Location-Feld des Termins, ueber `calendarReader.updateEvent(event.id, {...})` — identisch fuer CalDAV- und EventKit-Binary-Backend.
...
- [ ] AC7: Bereits existierende Notizen mit leerem Location-Feld werden beim naechsten Oeffnen nachtraeglich verlinkt. Ein nicht-leerer bestehender Location-Wert bleibt stets unangetastet.
...
```gherkin
Scenario: Oeffnen einer bereits existierenden Notiz verlinkt nachtraeglich
  ...
  Then enthaelt das Location-Feld den Obsidian-Deeplink zu dieser Notiz
```
...
## Affected Areas
- `src/calendar-view.ts`: `openEvent` erweitert den `openOrCreate`-Aufruf um den automatischen Location-Deeplink-Schreibvorgang, wenn `event.location` leer ist.
...
- Automated: `openEvent`-Pfad ruft bei leerem `event.location` fuer neue und bestehende Notizen `calendarReader.updateEvent` mit vollstaendigem `EventUpdate` (Titel, Start, Ende, bestehende `notes`, bestehender `calendar`, neuer `location`-Deeplink) auf.
