...
function collectFindings(report, cwd) {
  ...
  for (const fileResult of report) {
    const file = relative(cwd, fileResult.filePath);
...
export function formatQualitySummaryMarkdown(summary) {
  ...
  const lines = ["## Code Quality Baseline (sonarjs)", "", `**Findings:** ${total}`, ""];

  const rules = Object.entries(byRule).sort(([, left], [, right]) => right - left);
  if (rules.length === 0) {
